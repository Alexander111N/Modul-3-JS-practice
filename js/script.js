...
//     body: JSON.stringify({name:'Alexey'}), // то что отправляем на сервер, если нужно
//     headers: { 
//         'Content-type': 'application/json'
//     } // Заголовки, которые определяют какой контент мы отправляем. так как заголовкИ, то мы используем объект.
//   })
//   .then(response => response.json())
//   .then(json => console.log(json))
...
//   const arrName = ['aaa','ffffff','  wewewe'];
//   const filteredName = arrName.filter((name) => {return name.length < 5;});
//   console.log(filteredName);

//   const arrName = ['aAa','ffffFFFf','weWWewe'];
//   const filteredName = arrName.map(item => item.toLocaleLowerCase());
//   console.log(arrName);

//   const arrName = ['aAa',123,'weWWewe'];
//   console.log(arrName.some(item => typeof(item) === 'number'));

  const arrName = [1, 16, 32];
  console.log(arrName.reduce((sum, item) => sum + item));
...
//   });
  ...
  const newArr = Object.entries(obj).map(item => {
    if(item[1] === 'person') 
        return item[0];
    else
        return 'Зюзя'
